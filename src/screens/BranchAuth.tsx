...
const BranchAuth: React.FC = ({navigation}) => {
  const [form, setForm] = useState({
    name: '', // Changed from branchName to name
    branchLocation: '',
    branchAddress: '',
    branchEmail: '',
    openingTime: '',
    ...
    ownerName: '',
    govId: '',
    deliveryServiceAvailable: 'yes' as 'yes' | 'no', // Changed from homeDelivery to deliveryServiceAvailable
  ...
  const formatData = useCallback(() => {
    ...
    const formattedLocation = JSON.stringify({
      type: 'Point',
      coordinates: [Number(locationParts[1]), Number(locationParts[0])], // Note: longitude comes first
    });
    ...
    return {
      name: form.name, // Changed from branchName to name
      ...
      govId: form.govId,
      deliveryServiceAvailable: form.deliveryServiceAvailable === 'yes', // Changed from homeDelivery to deliveryServiceAvailable
  ...
  const handleNext = useCallback(() => {
    try {
      ...
      if (
        !form.name || // Changed from branchName to name
        !form.branchLocation ||
        !form.branchAddress ||
        !form.openingTime ||
        !form.closingTime ||
      ...
      navigation.navigate('PhoneNumberScreen', {formData: formattedData});
    } catch (error) {
  ...
  return (
    <View style={styles.container}>
      <TextInput
        ...
        value={form.name} // Changed from branchName to name
        onChangeText={text => setForm(prev => ({...prev, name: text}))} // Changed from branchName to name
        style={styles.input}
      />
      <TextInput
      ...
      <Picker
        selectedValue={form.deliveryServiceAvailable} // Changed from homeDelivery to deliveryServiceAvailable
        onValueChange={value =>
